...
class AdaGrad(Optimizer):
    ...
    def update(params, state, batch):
        ...
        updated_grads = tree_map(lambda precond, g: g/jnp.sqrt(precond), state['G'], grads)

        return tree_map(
            lambda p, g: p - eta * g, params, updated_grads
        ), state
...
class SpiderBoost(Optimizer):
    ...
    def update(params, state, batch):
        ...
        V = tree_map(
            lambda curr_grad, prev_grad, v: curr_grad - prev_grad + v, grads, grads_prev, V
        )

        state['V'] = V
        return tree_map(
            lambda p, g: p - step_size * g, params, V
        ), state
